...
> 1. **Bottom-nav ripple:** 28 previews across 7 screens (account-detail/accounts/beneficiaries/home/transaction-detail/transactions/transaction-tags) still show Pay→send-money; re-render their bottom nav to Pay→pay (app-shell already Pay→pay), then retire `screens/send-money/`. (No-sed rule precludes a batch script — re-render via `/idea-render-screen` per screen, or hand-edit the single `nav_pay` line per file via Edit.)
