...
|--------------------|--------------------|---------------------|--------|----------|------------------------------|
| agent-registration | Agent Registration | /agent-registration | Column | Vertical | AgentRegistrationViewModel   |

**States handled:** loading, idle, submitting, validation_error, pending_approval, confirmed, content, empty, error
...
| registration_loading_spinner| loading_indicator | 48dp circular indicator, `#4C662B`; centred with 80dp top margin; shown during pre-flight OBP status check; `role: progressbar`, `live: polite` |
| loading_label               | text              | "Checking registration status…" — body_medium, `#44483D`, centred; 8dp bottom margin; shown below spinner; `live: polite`                 |
| status_banner_pending       | box               | `#CDEDA3` bg, `#E8A317` border (1dp), 14dp radius, 16dp H / 14dp V padding, 20dp H / 20dp bottom margin; `role: status`, `live: polite`   |
| pending_banner_row          | stack             | Horizontal row, 10dp spacing, aligned centre — holds `pending_icon` + `pending_text_col`                                                   |
| pending_icon                | icon              | `hourglass_empty`, 22dp, `#44483D` (a11y-corrected; ≥7.25:1 contrast on #CDEDA3); decorative (`role: none`)                               |
| pending_text_col            | stack             | Vertical column (flex 1) — holds `pending_banner_title` + `pending_banner_message`; `role: group`                                         |
...
| status_banner_confirmed     | box               | `#CDEDA3` bg, `#CDEDA3` border (1dp), 14dp radius, 16dp H / 14dp V padding, 20dp H / 20dp bottom margin; `role: status`                   |
...
| legal_name_input            | input             | Outlined text field; placeholder "e.g. Priya Chakraborty"; 12dp radius; `#E1E4D5` border; maps to OBP `legal_name`; `role: textbox`        |
...
| phone_input_row             | stack             | Horizontal, 8dp gap, 20dp H margin — contains `phone_prefix_box` + `phone_number_input`; `role: row`                                       |
...
| currency_select             | input             | Combobox (`role: combobox`), `expand_more` trailing icon; options: EUR — Euro, GBP — British Pound, KES — Kenyan Shilling, USD — US Dollar |
...
| services_chip_group         | stack             | Wrap chip group (variant: chip_group), 8dp gap, 20dp H margin; chips: Cash Deposit / Cash Withdrawal / Account Opening / Bill Payment / Fund Transfer. Selected: `#4C662B` bg / `#FFFFFF` text; unselected: `#CDEDA3` bg / `#4C662B` text / `#4C662B` border (1dp) / 20dp radius; `on_click: toggle_service` |
...
| global_error_banner         | box               | `#CDEDA3` bg, `#BA1A1A` border (1dp), 12dp radius, 16dp H / 12dp V padding, 20dp H / 16dp bottom margin; `role: alert`, `live: assertive` |
| global_error_row            | stack             | Horizontal, 10dp spacing, centre-aligned — `error_icon` + `global_error_message`; `role: row`                                              |
...
| register_agent_button       | button            | "Register as Agent" — filled, `#4C662B` bg / `#FFFFFF` text, full-width, 14dp radius, label_large, 24dp H / 16dp V padding, 2dp elevation; disabled + loading during `submitting` state; `on_click: submit_registration → obp_create_agent` |
...
| confirmed        | OBP pre-check or POST returned `is_confirmed_agent=true` | Confirmed banner + "Go to Dashboard" button; form inputs hidden                                              |
...
| colors.light.on_surface_variant | `#44483D`             | Subtitle text, all field labels, pending/confirmed banner body, a11y-corrected icon colors      |
...
| radius.lg                       | 14dp                  | Button and status banner radius                                                                 |
...
| elevation.level2                | 2dp                   | Register + Go to Dashboard button elevation                                                     |
